...
def convert_example_to_feature(
    ...
    if example.text_b:
        ...
        # special_tokens的数量：
        # bert： [CLS], A[SEP], B[SEP]    3.
        # RoBERTa：<s> A </s></s> B </s>  4.
        # https://github.com/pytorch/fairseq/commit/1684e166e3da03f5b600dbb7855cb98ddfcd0805
...
def convert_example_to_feature_sliding_window(
    ...
    (example, max_seq_length, tokenizer, output_mode, cls_token_at_end,
     cls_token, sep_token, cls_token_segment_id, pad_on_left,
     pad_token_segment_id, sep_token_extra, multi_label, stride) = example_row
    ...
    if len(tokens_a) > bucket_size:
        token_sets = [
            tokens_a[i:i + bucket_size]
            for i in range(0, len(tokens_a), stride)
        ]
    else:
...
def convert_examples_to_features(
    ...
    if use_multiprocessing:
        if sliding_window:
            ...
            if flatten:
                features = [
                    feature for feature_set in features
